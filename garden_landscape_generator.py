...
class GardenLandscapeGenerator:
    ...
    def generate_summary_report(self, results):
        ...
        fig.suptitle('江南古典园林景观元素分布分析总结', fontsize=16, fontweight='bold')
        ...
        ax1.set_xlabel('园林')
        ax1.set_ylabel('元素数量')
        ax1.set_title('各园林景观元素数量对比')
        ax1.set_xticks(x_pos + width*2.5)
        ax1.set_xticklabels(garden_names, rotation=45, ha='right')
        ...
        ax2.set_xlabel('园林')
        ax2.set_ylabel('占地面积 (平方米)')
        ax2.set_title('各园林占地面积对比')
        ...
        ax3.scatter(areas, densities, c='purple', alpha=0.6, s=100)
        ax3.set_xlabel('占地面积 (平方米)')
        ax3.set_ylabel('元素密度 (个/平方米)')
        ax3.set_title('园林规模与元素密度关系')
        ax3.grid(True, alpha=0.3)

        for i, name in enumerate(garden_names):
            ax3.annotate(name[0], (areas[i], densities[i]),
                        fontsize=9, ha='center', va='center')

        # 4. 各类元素占比饼图（以所有园林总和为基准）- 修复后的代码
        ...
        ax4.set_title('所有园林景观元素类型分布')

        plt.tight_layout()

        # 保存总结图表
        summary_filename = "results/landscape_maps/园林景观元素分析总结.png"
